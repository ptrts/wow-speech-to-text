...
@pytest.mark.parametrize("name, calls, result", [
    ...
    ("В финале повторяется последнее частичное, потом продолжение", [(False, "зима"), (False, "зима осень"), (True, "зима осень"), (False, "весна"), (False, "весна лето")], "Зима осень весна лето"),
    ...
    ("Добавка многословной команды должна удаляться одним удалить за раз", [(True , "зима лето точка с запятой осень удалить удалить")], "Зима лето"),
    ...
    ("Удаление команды не приходит во второй раз", [(False, "зима открывающая скобка удалить лето"), (False, "зима открывающая скобка утолить лето")], "Зима (утолить лето"),
    ...
    ("Запятая запятая", [(False, "зима запятая запятая лето")], 'Зима, лето'),
